...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (firstKickoff && Date.now() >= firstKickoff) {
      return json(400, { error: 'This round is locked. Picks close at the first game kickoff.' });
    }

    if (submittedIds.size !== roundIds.length || !roundIds.every(id => submittedIds.has(id))) {
      return json(400, { error: `Please predict every match in ${round} before locking in your picks.` });
    }
    ...
    if (invalid) return json(400, { error: 'Please choose a winner/draw and enter both scores for every match.' });
    ...
    if (existingPredictions && existingPredictions.length > 0) {
      return json(400, { error: `You have already locked in picks for ${round}. Picks are final once submitted.` });
    }

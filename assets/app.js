...
function renderMatches(active=roundNames()[0]){
  ...
  const grouped={};
  matches.forEach(m=>{const day=fmtDay(m.kickoff); if(!grouped[day]) grouped[day]=[]; grouped[day].push(m);});
  const lockNotice=lockTime?`<div class="round-lock open-notice">⏳ Predict every match in this round. Picks lock at the first kickoff: ${fmtFull(lockTime)} Kuwait time. Once submitted, picks are final.</div>`:'';
